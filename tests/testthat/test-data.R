...
test_that("time series data can be imported", {
  set.seed(0)
  methods <- c(
    "kmeans",
    "kde",
  ...
  for (m in methods) {
    ...
    expect_true(
      all(unlist(imported[[m]]$sequence_data) %in% 1:3)
    )
    expect_true(
      nrow(imported[[m]]$sequence_data) == n
    )
    expect_true(
      ncol(imported[[m]]$sequence_data) == k
    )
  }
})
...
test_that("missing data columns fail", {
  expect_error(
    ...
    "The columns \"series\" and \"not_id\" must exist in the data"
  )
})
...
test_that("invalid number of states fails", {
  expect_error(
    ...
    "Argument `n_states` must be an integer greater than 1\\."
  )
})
