...
TNA is intended for analysis of ordered sequential data (i.e., sequences). However, it supports the conversion from time series data (i.e., ordered continous data) to the format necessary for TNA.
...
head(EuStockMarkets)
```
Now we use the `prepare_ts` function included in `tna` to import the data. We just need to specify how many discrete states we want to consider.
```{r}
data_ts <- prepare_ts(EuStockMarkets, n_states = 4)
```

Now we can plot our time series together with the assigned states at each time point by using the `plot` function and passing the result of `prepare_ts` as an argument.
```{r}
plot(data_ts)
```
...
```
The `prepare_ts` function also takes time series data in a `data.frame` (in long format). Let us create an example with 10 time series with 100 points each.
```{r}
...
```{r}
plot(ts_data_long, selected = c(2, 3), points = TRUE, overlay = FALSE)
```
